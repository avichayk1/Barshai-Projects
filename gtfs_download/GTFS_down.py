...
# Configure logging
logging.basicConfig(
    filename='gtfsdownload.log',  # Log file name
...
def compare_text_files_using_sets(file1_path, file2_path, relative_path):
    ...
    logging.error(f"run compare_text_files {basename}")
    ...
    logging.error(f"finish compare {os.path.basename(file2_path)}")
def compare_text_files(file1_path, file2_path, relative_path):
    print(f"run compare_text_files {os.path.basename(file1_path)}")
    logging.error(f"run compare_text_files {os.path.basename(file1_path)}")
...
move.manage_folders(gtfs_weekly_directory)
os.makedirs(new_gtfs_directory, exist_ok=True)
os.makedirs(change_log_path, exist_ok=True)
...
for root, _, files in os.walk(gtfs_directory):
    if os.path.basename(root) == 'logs':  # Check if the current directory is 'logs'
        t=0
    else:  # Check if the current directory is not 'logs'
        for file in files:
...
if not os.path.exists(change_log_path+ "directory"+"_change_log"):
    open(change_log_path+ "/directory"+"_change_log.txt", 'w', encoding='utf-8').close()
log_file_handler = open(os.path.join(change_log_path, "directory"+"_change_log.txt"), 'a', encoding='utf-8')


def log_directory_differences(dcmp,full_path):
    for name in dcmp.left_only:
        log_changes(f'[{get_current_datetime()}] New file or directory: {name}',full_path)
    for name in dcmp.right_only:
        log_changes(f'[{get_current_datetime()}] File or directory deleted: {name}',full_path)
    for name in dcmp.diff_files:
        log_changes(f'[{get_current_datetime()}] Difference in file: {name}',full_path)
